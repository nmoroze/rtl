...
(begin-for-syntax
  ...
  (define-splicing-syntax-class yosys-member
    ...
    (pattern (~seq (name:id type:yosys-type) base-external-name:id)
             #:with show #'type.show
             #:with external-name (let ([pieces (string-split (symbol->string (syntax->datum #'name)) "#")])
                                    (if (= (length pieces) 3)
                                        (format-id #f "~a#~a" (syntax->datum #'base-external-name) (third pieces))
                                        #'base-external-name))
